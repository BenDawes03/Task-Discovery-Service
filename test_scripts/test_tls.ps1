...
function Test-TLSRegistrationAndQuery {
    ...
    if (-not (Test-Path "cmd\client_demo\client_demo.exe")) {
...
try {
    `$output = & '.\cmd\client_demo\client_demo.exe' -tls -cert '$CertDir/client.crt' -key '$CertDir/client.key' -ca '$CertDir/ca.crt' 2>&1
    if (`$LASTEXITCODE -eq 0) {
...
function Test-ConcurrentTLSConnections {
    ...
    for ($i = 1; $i -le $numConcurrent; $i++) {
        $job = Start-Job -ScriptBlock {
            ...
            $output = & ".\cmd\client_demo\client_demo.exe" -tls `
