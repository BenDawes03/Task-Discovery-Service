...
\section{Design}
\subsubsection{services, tasks and the client proxy}
...
\begin{lstlisting}
{"status":"OK"}
{"status":"OK","address":"10.0.1.5:8080"}
{"status":"NOTFOUND"}
{"status":"ERR","error":"task required"}
\end{lstlisting}

\textbf{Validation and error cases}
\begin{itemize}
    \item Missing task on query returns \texttt{\{"status":"ERR","error":"task required"\}}
    \item Missing task or address on register returns \texttt{\{"status":"ERR","error":"task and address required"\}}
    \item Invalid capacity returns \texttt{\{"status":"ERR","error":"capacity must be a positive integer"\}}
    \item Unknown command returns \texttt{\{"status":"ERR","error":"unknown command"\}}
    \item Malformed JSON returns \texttt{\{"status":"ERR","error":"invalid JSON: <parse error>"\}}
    \item Non-JSON input returns \texttt{\{"status":"ERR","error":"JSON required"\}}
\end{itemize}
...
Introduce the server, cache and database. 
