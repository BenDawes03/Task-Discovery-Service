...
```powershell
\.\bin\server.exe --store-url="postgresql://postgres:mysecretpassword@localhost:5432/tds?sslmode=disable" --cache-max-size=10 --tcp
```

### 3. Run the Cache Demo
In a separate terminal:
```powershell
go run ./cmd/cache_demo
```
...
Edit [cmd/cache_demo/main.go](../../cmd/cache_demo/main.go) to customize:
...
Source: [cmd/cache_demo/main.go](../../cmd/cache_demo/main.go)
