...
$nodeA = Start-Process -FilePath ".\bin\client_proxy.exe" -ArgumentList "-p2p","-p2p-port",":6000" -PassThru -WindowStyle Normal

Start-Sleep -Seconds 2

Write-Host "Starting Node B on :6001 (client proxy on :5101)..." -ForegroundColor Yellow
$env:TDS_PROXY_LISTEN=":5101"
$nodeB = Start-Process -FilePath ".\bin\client_proxy.exe" -ArgumentList "-p2p","-p2p-port",":6001","-bootstrap","localhost:6000" -PassThru -WindowStyle Normal

Start-Sleep -Seconds 2

Write-Host "Starting Node C on :6002 (client proxy on :5102)..." -ForegroundColor Yellow
$env:TDS_PROXY_LISTEN=":5102"
$nodeC = Start-Process -FilePath ".\bin\client_proxy.exe" -ArgumentList "-p2p","-p2p-port",":6002","-bootstrap","localhost:6000" -PassThru -WindowStyle Normal
